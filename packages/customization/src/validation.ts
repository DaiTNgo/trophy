...
const hasLocalizedContent = (val: unknown): boolean => {
  if (typeof val === "string") return val.trim().length > 0;
  if (!val || typeof val !== "object") return false;
  const loc = val as { vi?: string; en?: string };
  return Boolean(loc.vi?.trim() || loc.en?.trim());
};
...
export const validateProductCustomizationForPublish = (customization: ProductCustomization) => {
  ...
  for (const field of customization.formFields) {
    if (!hasLocalizedContent(field.label)) {
      issues.push({
    ...
    if ((field as any).type === "select") {
      for (const choice of (field as any).choices ?? []) {
        if (!hasLocalizedContent(choice.label)) {
          issues.push({
            code: "LOCALIZATION_INCOMPLETE",
            fieldId: field.id,
            message: `${(field.label as any)?.vi || (field.label as any)?.en || "Field"} choice "${(choice.label as any)?.vi || (choice.label as any)?.en || "Choice"}" is missing required label.`,
          });
        }
      }
    }
