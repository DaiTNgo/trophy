...
### Requirement: Zone Customization Blocks

The system SHALL allow an administrator to define ordered typed blocks inside each customization zone.

#### Scenario: Define a text block

- **WHEN** an administrator adds a text or textarea block
- **THEN** the administrator configures its label, required state, character limit, line limit, font policy, default value, and fixed placement
- **AND** a text block is restricted to one line while a textarea block is restricted to its configured `maxLines`

#### Scenario: Define a preset media block

- **WHEN** an administrator adds a logo, background, border, or artwork selector
- **THEN** the administrator configures revisioned asset options and at most one default option
...
- **THEN** the administrator can define a visibility condition against an earlier select, radio, or media-select block
...
- **AND** no shopper value can override position, size, rotation, or z-index
